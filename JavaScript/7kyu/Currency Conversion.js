...
function convertMyDollars(usd, currency) {
    const displayed = CONVERSION_RATES[currency];
    const first_char = currency[0].toLowerCase();
    const is_vowel = ['a','e','i','o','u'].includes(first_char);
  
    const rate = is_vowel ? Number(displayed) : parseInt(String(displayed), 2);
    const exchanged = usd * rate;
  
    return `You now have ${exchanged} of ${currency}.`;
}
