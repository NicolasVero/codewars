...
export function binaryFingers(bin_string: string): string[] {
	const finger_order = ['Thumb', 'Index', 'Middle', 'Ring', 'Pinkie'] as const;
	const result: string[] = [];

	for (let i = 0; i < bin_string.length; i++) {
		if (bin_string[i] === '1') {
			const finger_index = bin_string.length - 1 - i;
			result.push(finger_order[finger_index]);
		}
	}
